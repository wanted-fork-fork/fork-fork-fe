...
import { GroupHistoryResponse } from 'src/types';

export const GroupHistoryPage = ({
  groupId,
  historyList,
}: {
  groupId: string;
  historyList: GroupHistoryResponse[];
}) => {
  return (
      ...
      <ScrollView>
        <Flex gap={8} direction={'vertical'}>
          {historyList.map((history) => (
            <GroupHistoryRow key={history.timestamp} groupId={groupId} history={history} />
          ))}
